      ...
      <Slide v-for="lieu in filteredLieux" :key="lieu.id">
        <div class="flex flex-col md:flex-row text-center mx-auto max-w-4xl p-4">
          ...
          <div class="md:w-1/2 p-4 flex flex-col justify-center">
            ...
            <a :href="lieu.link" class="mt-2 inline-block bg-[#763538] text-white rounded-md py-3 px-6 hover:bg-white hover:text-[#763538] transition-colors">En Savoir plus ...</a>
          </div>
        </div>
      </Slide>
    </Carousel>
...
export default defineComponent({
  ...
  computed: {
    ...
    actualItemsToShow() {
      return this.filteredLieux.length < this.itemsToShow ? this.filteredLieux.length : this.itemsToShow;
    }
  },
  mounted() {
    this.fetchData();
